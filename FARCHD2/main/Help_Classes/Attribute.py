...
# '''
 ...
 def increaseClassFrequency(self, whichClass,  value):
  ...
  if self.__makeStatistics and self.__classFrequencies is not None and self.__classFrequencies[whichClass] is not None:
 ...
 def addInMeanValue(self, whichClass,  value):
  if self.__makeStatistics:
   self.__numStatUpdates [whichClass] = self.__numStatUpdates [whichClass] + 1
   self.__meanValue[whichClass] = self.__meanValue[whichClass] + value
 ...
 def normalizeValue (self,val):
  ...
  if self.__type == self.REAL:
   return (val-self.__min)/(self.__max-self.__min)
  return val
 #end normalizeValue
  ...
  if self.__type==self.NOMINAL:
    ...
    for  i in range (0,len(self.__nominalValues)):
     if i == len(self.__nominalValues) - 1:
      ending = ""
     aux = aux + str(self.__nominalValues[i]) + ending
    ...
    #//System.out.println("name->" + name);
      ...
      aux = aux + " integer[" + str(int(self.__min))
      aux = aux + ","+ str(int(self.__max))+"]"

  elif self.__type== self.REAL:
      aux = aux + " real["+ str(float( self.__min))
      aux = aux + ","+ str(float(self.__max))+"]"
